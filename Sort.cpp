...
void Merge(int *a, int l, int m, int r, ll &cc)
{
    ...
    for (int i = 0; ++cc && i < n1; i++)
    {
        temp1[i] = a[l + i];
    }
    for (int i = 0; ++cc && i < n2; i++)
    {
        temp2[i] = a[m + i + 1];
    }
...
void BubbleSort(int *a, int n, ll &cc)
{
    ...
    for (int i = 0; ++cc && i < n - 1; i++)
    {
        for (int j = 0; ++cc && j < n - i - 1; j++)
        {
            if (++cc && a[j] > a[j + 1])
            {
                int temp = a[i];
...
void shellSort(int arr[], int n, ll &cc)
{
    ...
    for (int gap = n / 2; ++cc && gap > 0; gap /= 2)
    {
        // Do a gapped insertion sort for this gap size.
        // The first gap elements a[0..gap-1] are already in gapped order
        // keep adding one more element until the entire array is gap sorted
        for (int i = gap; ++cc && i < n; i += 1)
        {
            ...
            int j;
            for (j = i; ++cc && j >= gap && ++cc && arr[j - gap] > temp; j -= gap)
            {
                arr[j] = arr[j - gap];
            }
...
void radixSort(int a[], int n, ll &cc)
{
    ...
    for (int exp = 1; ++cc && k / exp > 0; exp *= 10)
    {
